...
<script type="text/javascript">
	$(function (){
		//分页
		var currentPage = ${CarouselFigurePages.page};
		var pageSize = ${CarouselFigurePages.size};
		var totalPage = ${CarouselFigurePages.getLocalPage()};
		$("#pagination3").pagination({
			...
			callback: function(current) {
				$('.hp-context').load("${ctx}/admin/carousels/list?page=" + current);
			}
		});
		...
		$('.bt_add').on('click', function(){
			layer.open({
				title: "添加轮播图",
			  	...
			  	fixed: false, //不固定
			  	maxmin: true,
			  	content: basePath + '/admin/carousels/addPage'
			});
		})
		...
		$('.bt_update').on('click', function(){
			...
			layer.open({
				title: "修改轮播图",
			  	...
			  	fixed: false, //不固定
			  	maxmin: true,
			  	content: basePath + '/admin/carousels/updatePage?id='+id
			});
		})
			...
			if (cks.length==0) {
				...
				layer.open({
					...
					yes: function(){
						...
						$.post('/admin/carousels/deleteAll', {
							ids: ids.join()
						}, function (e) {
							if (e=="0") {
								$('.hp-context').load("${ctx}/admin/carousels/list");
		...
		<table>
			...
			<c:forEach items="${CarouselFigurePages.list }" var="carouselfigure">
				<tr>
					...
					<td><img alt="" src="${ctx}/common/getImage?name=${carouselfigure.url }" height="50px;"></td>
